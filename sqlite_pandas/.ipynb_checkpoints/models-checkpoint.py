...
class sqlite_pandas(object):
    ...
    def get_data(self, table_name):
        try:
            return [row for row in self.c.execute('select * from {}'.format(table_name))]
        except:
            return None

    # tableカラム名を取得
    def get_tblcol(self, table_name):
        self.c.execute("select * from sqlite_master where tbl_name='{}'".format(table_name))
    ...
    def delete_data(self, table_name, key, query):
        ...
        choice = input("above data will be deleted. 'yes' or 'no' [y/N]: ").lower()
        if choice in ['y', 'ye', 'yes']:
            sql = 'delete from {} where {} = "{}";'.format(table_name, key, query)
            self.c.execute(sql)
            print('deleted')
        elif choice in ['n', 'no']:
            print('stopped')
